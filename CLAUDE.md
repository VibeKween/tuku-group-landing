...
- **Status (2026-08-26)**: Fully live on `tukugroup.com` — backend (D1/R2/KV) and frontend (gate/field/reader) both merged to `main` and deployed; `/clients/full-charge/` serves the real gate/field/reader, not the placeholder. Field module imports (`board.js`/`reader.js`/`reader.css`) are cache-busted with a versioned query string so mobile browsers can't keep running a stale cached JS module after a deploy (they carry a 4h `Cache-Control` while the HTML itself revalidates on every load). Full status: `docs/builds/full-charge-client-archive.md`.
