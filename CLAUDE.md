...
- **Status (2026-08-25)**: Backend is **live on `tukugroup.com`** (D1/R2/KV provisioned and seeded; deployed to production with the real route mounted; unlock/board/artifact/lock/admin all verified end-to-end directly against the real domain). Frontend (gate/field/reader) is built and tested but only exists on the `dev` branch — Cloudflare Pages deploys `main`, so the actual client-facing pages aren't live yet; `/clients/full-charge/` still serves the old placeholder until that merge happens. Full status: `docs/builds/full-charge-client-archive.md`.
